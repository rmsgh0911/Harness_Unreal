...
def run_doctor(root: Path) -> dict:
    ...
    required_files = [
        ...
        harness / "index" / "api_surface.md",
        harness / "index" / "verification_map.md",
    ...
    legacy_worker_dirs = [root / "Harness" / name for name in ["Codex", "Claude", "Common"]]
    results.append(
        check(
            not any(path.exists() for path in legacy_worker_dirs),
            "split worker Harness directories are removed after migration",
        )
    )
    ...
    for path in json_paths:
        ...
        if path.name == "agents.json" and isinstance(data, dict):
            workers = data.get("supported_workers", {})
            results.append(check(isinstance(workers, dict) and bool(workers), "agents.json has supported_workers"))
            if isinstance(workers, dict):
                for worker, worker_data in workers.items():
                    instruction = worker_data.get("instruction_file", "") if isinstance(worker_data, dict) else ""
