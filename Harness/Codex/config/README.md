...
`docs.json` stores document roots, the policy for when agents should read docs, and request hints. Do not place full design docs under `Harness/Codex/config/`.
...
`Harness/Codex/Progress.md` is the default Korean human-facing dashboard. Other Harness operating files should stay in English for agent portability.
...
Worker switching should be explicit. When a switch happens, record the reason and the new worker's first-read files in today's `Harness/Codex/work/cycles/YYYY-MM-DD.md`.
