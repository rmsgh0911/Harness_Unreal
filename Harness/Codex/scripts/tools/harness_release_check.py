...
DUPLICATED_WORK_PATHS = ("Harness/Codex/work/work/", "Harness/Claude/work/work/")
...
def build_report(root: Path, strict: bool = False) -> dict:
    ...
    for old_path in [harness / name for name in ["config", "scripts", "index", "work", "docs", "doc"]]:
        if old_path.exists():
            errors.append({"path": rel(old_path, root), "message": "legacy_single_worker_layout_path"})
    ...
    cycle_files = sorted(
        ...
        for path in (harness / worker / "work" / "cycles").glob("*.md")
