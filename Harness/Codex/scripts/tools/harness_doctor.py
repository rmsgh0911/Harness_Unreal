...
def run_doctor(root: Path) -> dict:
    ...
    required_files = [
        ...
        root / "Harness" / "Common" / "README.md",
        root / "Harness" / "Common" / "policies" / "README.md",
        root / "Harness" / "Claude" / "README.md",
        root / "Harness" / "Claude" / "Progress.md",
        root / "Harness" / "Claude" / "config" / "project.json",
        root / "Harness" / "Claude" / "work" / "state.md",
        root / "Harness" / "Claude" / "work" / "next.md",
        root / "Harness" / "Claude" / "scripts" / "tools" / "tool_manifest.json",
        harness / "README.md",
        ...
        root / "Harness" / "Common" / "docs" / "README.md",
    ...
    legacy_worker_dirs = [
        root / "Harness" / name for name in ["config", "scripts", "index", "work", "docs"]
    ]
    results.append(
        check(
            not any(path.exists() for path in legacy_worker_dirs),
            "legacy single-worker Harness directories are removed after migration",
        )
    )
    ...
    results.append(check(includes(ag_text, "Harness/Codex/"), "AGENTS.md routes Codex to its worker area"))
    if (root / "CLAUDE.md").exists():
        results.append(check(includes(cl_text, "HARNESS.md"), "CLAUDE.md routes Claude Code to HARNESS.md"))
        results.append(check(includes(cl_text, "Harness/Claude/"), "CLAUDE.md routes Claude Code to its worker area"))
    results.append(check(includes(install_text, "## Configure Codex"), "INSTALL.md configures Codex"))
    results.append(check(includes(install_text, "## Configure Claude Code"), "INSTALL.md configures Claude Code"))
    results.append(check(includes(install_text, "Harness/Common/"), "INSTALL.md explains shared material"))
    ...
    results.append(check(includes(harness_text, "Harness/Common/docs"), "HARNESS.md explains default project document root"))
    results.append(check(includes(harness_text, "Harness/Codex/"), "HARNESS.md explains Codex worker area"))
    results.append(check(includes(harness_text, "Harness/Claude/"), "HARNESS.md explains Claude Code worker area"))
    ...
    for path in json_paths:
        ...
        if path.name == "tool_manifest.json" and isinstance(data, dict):
            ...
            for index, tool in enumerate(tools, start=1):
                ...
                results.append(check(str(declared_path).startswith("Harness/Codex/scripts/tools/"), f"manifest tool stays under tools: {name}"))
