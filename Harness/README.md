...
- `state.md`는 누적 작업 일지가 아니다.
- 최신 확정 상태만 현재형으로 유지한다.
- 오래된 이력, 시도, 실패, 리뷰 과정은 `cycles/` 또는 `reviews/`에 둔다.
- 완료된 `next.md` 항목은 `state.md`의 현재 사실로 반영한 뒤 `next.md`에서 제거한다.
...
7. `cycles/YYYY-MM-DD.md`에 결과를 남기고, 현재 상태로 확정된 사실만 `state.md`에 반영한다.
