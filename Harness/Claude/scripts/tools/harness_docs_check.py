...
def is_harness_docs_path(value: str) -> bool:
    ...
    return normalized == "Harness/Common/docs" or normalized.startswith("Harness/Common/docs/")
...
def build_report(root: Path, request: str = "") -> dict:
    ...
    for doc_root in doc_roots:
        ...
        if inside_harness and not is_harness_docs_path(doc_root):
            add_finding(findings, "error", "project documents inside Harness must use Harness/Common/docs", doc_root)
    ...
    if (root / "Harness" / "doc").exists():
        add_finding(findings, "warning", "legacy Harness/doc exists; migrate project documents to Harness/Common/docs", "Harness/doc")
