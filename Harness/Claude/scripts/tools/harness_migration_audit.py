...
ACTIVE_DOCS = [
    ...
    "README.md",
    "Harness/README.md",
    "Harness/Claude/work/state.md",
    "Harness/Claude/work/next.md",
]
OLD_PATH_PATTERNS = [
    "Harness/Claude/scripts/verify_project.py",
    "Harness/Claude/scripts/create_level.py",
    "Harness/Claude/scripts/build_verify.ps1",
    "Harness/Claude/scripts/build_verify.cmd",
]
NEW_PATH_REPLACEMENTS = {
    "Harness/Claude/scripts/verify_project.py": "Harness/Claude/scripts/unreal/verify_project.py",
    "Harness/Claude/scripts/create_level.py": "Harness/Claude/scripts/unreal/create_level.py",
    "Harness/Claude/scripts/build_verify.ps1": "Harness/Claude/scripts/build/build_verify.ps1",
    "Harness/Claude/scripts/build_verify.cmd": "Harness/Claude/scripts/build/build_verify.cmd",
}
...
def find_custom_root_scripts(root: Path) -> list[str]:
    worker_scripts = root / "Harness" / "Claude" / "scripts"
...
def this_installation_root() -> Path:
    ...
    up-to-date installation when auditing an older project:
        ...
        python /path/to/new/project/Harness/Claude/scripts/tools/harness_migration_audit.py \\
...
def project_config_summary(root: Path) -> dict:
    worker_config = root / "Harness" / "Claude" / "config" / "project.json"
...
def audit(root: Path) -> dict:
    ...
    if existing(work / "state.md"):
        preserve.append("Harness/Claude/work/state.md" if has_worker_layout else "Harness/work/state.md")
        ...
        update.append("move Harness/state.md to Harness/Claude/work/state.md")
        preserve.append("Harness/state.md")
    if existing(work / "next.md"):
        preserve.append("Harness/Claude/work/next.md" if has_worker_layout else "Harness/work/next.md")
        if not has_worker_layout:
        ...
        update.append("move Harness/next.md to Harness/Claude/work/next.md")
        preserve.append("Harness/next.md")
    if cycle_files:
        preserve.append("Harness/Claude/work/cycles/" if has_worker_layout else "Harness/work/cycles/")
        if not has_worker_layout:
        ...
        update.append("move Harness/cycles/ to Harness/Claude/work/cycles/")
        preserve.append("Harness/cycles/")
    if index.exists():
        preserve.append("Harness/Claude/index/" if has_worker_layout else "Harness/index/")
        if not has_worker_layout:
    ...
    if legacy_doc.exists():
        ...
        update.append("migrate legacy Harness/doc project documents to Harness/Common/docs/ or register the existing folder in Harness/Claude/config/docs.json")
    ...
    if existing(config / "project.json"):
        preserve.append("Harness/Claude/config/project.json" if has_worker_layout else "Harness/config/project.json")
        if not has_worker_layout:
            update.append("copy or merge Harness/config/project.json into both worker project.json files")
    if existing(config / "docs.json"):
        preserve.append("Harness/Claude/config/docs.json" if has_worker_layout else "Harness/config/docs.json")
        if not has_worker_layout:
        ...
        update.append("add Harness/Claude/config/docs.json")
    ...
    if existing(harness / "backlog.md"):
        cleanup.append("Harness/backlog.md after moving unresolved items into Harness/Claude/work/next.md")
    ...
    if old_script_layout:
        update.append("move standard scripts to Harness/Claude/scripts/unreal/ and Harness/Claude/scripts/build/")
        findings.append({"level": "warning", "message": "standard scripts are in legacy Harness/Claude/scripts root"})
    if not existing(scripts / "tools" / "tool_manifest.json"):
        update.append("add Harness/Claude/scripts/tools/ and tool_manifest.json")
    ...
    if old_refs:
        update.append("replace old Harness/Claude/scripts/*.py and build_verify.* references in active docs")
    ...
    if not project["exists"]:
        findings.append({"level": "error", "message": "Harness/Claude/config/project.json is missing"})
