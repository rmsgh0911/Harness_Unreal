...
def build_plan(root: Path) -> dict:
    ...
    required = [
        ...
        "AGENTS.md",
        "Harness/README.md",
        "Harness/Claude/work/README.md",
        "Harness/Claude/work/state.md",
        "Harness/Claude/work/next.md",
        "Harness/Claude/index/README.md",
        "Harness/Claude/index/project_index.md",
        "Harness/Claude/config/project.json",
        "Harness/Claude/config/docs.json",
        "Harness/Claude/config/cycle_policy.json",
        "Harness/Claude/scripts/tools/tool_manifest.json",
    ]
    ...
    for item, path in [
        ("Harness/Claude/work/state.md", state_path(root)),
        ("Harness/Claude/work/next.md", next_path(root)),
        ("Harness/Claude/work/cycles/", cycles_dir(root)),
        ("Harness/Claude/config/project.json", harness / "config" / "project.json"),
    ]:
        if path.exists():
            preserve.append(item)

    if not project.get("project_name"):
        fill.append("Harness/Claude/config/project.json: project_name")
    if not project.get("uproject_file"):
        fill.append("Harness/Claude/config/project.json: uproject_file")
    build = project.get("build", {}) if isinstance(project, dict) else {}
    if not build.get("engine_root"):
        fill.append("Harness/Claude/config/project.json: build.engine_root")
    if not build.get("editor_target_name"):
        fill.append("Harness/Claude/config/project.json: build.editor_target_name")
    if not docs.get("doc_roots"):
        fill.append("Harness/Claude/config/docs.json: doc_roots")
    ...
    verify.extend(
        [
            ...
            "python Harness/Claude/scripts/tools/harness_doctor.py",
            "python Harness/Claude/scripts/tools/harness_scan.py --json",
        ]
    )
    if project.get("uproject_file") or len(uprojects) == 1:
        verify.append("python Harness/Claude/scripts/unreal/verify_project.py via UnrealEditor-Cmd")
