...
def state_specific(root: Path) -> dict:
    relative = "Harness/Claude/work/state.md"
...
def build_report(root: Path) -> dict:
    ...
    docs = [
        check_file(root, "Harness/Claude/work/state.md", soft_limit=140, hard_limit=220, allow_placeholders=template_unconfigured),
        check_file(root, "Harness/Claude/work/next.md", soft_limit=100, hard_limit=160, allow_placeholders=template_unconfigured),
    ...
    if cycles["large_files"]:
        findings.append({"level": "info", "path": "Harness/Claude/work/cycles/", "message": f"large cycle files: {len(cycles['large_files'])}"})
    if cycles["file_count"] > 45:
        findings.append({"level": "info", "path": "Harness/Claude/work/cycles/", "message": f"many cycle files: {cycles['file_count']}"})
    if cycles["total_lines"] > 1200:
        findings.append({"level": "info", "path": "Harness/Claude/work/cycles/", "message": f"large cycle history: {cycles['total_lines']} lines"})
    return {
        "root": str(root),
