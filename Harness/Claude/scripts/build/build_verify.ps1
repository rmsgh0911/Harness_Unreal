...
function Read-ProjectConfig {
    ...
    $configPath = Join-Path $projectDir "Harness\\Claude\\config\\project.json"
...
function Resolve-UprojectPath($ProjectDir, $Config) {
    ...
    if ($candidates.Count -ne 1) {
        Fail "Set uproject_file in Harness/Claude/config/project.json"
    }
...
function Resolve-EngineRoot($Config) {
    ...
    if (-not $engineRoot) {
        Fail "Set build.engine_root in Harness/Claude/config/project.json or UE_ENGINE_ROOT in the environment."
    }
