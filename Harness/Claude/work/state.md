# State

Agent-facing technical reference. Keep only the latest confirmed facts in present tense.
Do not use as a work log, history journal, or human dashboard — that is `Harness/Claude/Progress.md`.

## Project

- Project type: Unreal Engine project
- Project name: TODO
- Primary module: TODO
- Engine version: TODO
- `.uproject`: TODO
- Editor startup map: TODO
- Harness test map: TODO

## Current Confirmed State

- TODO: Record systems, maps, inputs, characters, controllers, UI, plugins, and assets that actually exist.
- TODO: Prefer Source, Config, editor output, and build logs over design intent.

## Project Docs

- TODO: If design docs, specs, simulation scenarios, or validation criteria exist, record whether they live in `Harness/Common/docs/` or a registered external docs folder in `Harness/Claude/config/docs.json`.
- `Harness/Claude/Progress.md` is a Korean human-facing dashboard, not a technical state source.

## Key Files

- TODO: `Source/<Module>/...`
- TODO: `Config/...`
- TODO: `Content/...`
- TODO: `Plugins/...`

## Verification State

- Last C++ build: TODO
- Last Harness script syntax check: TODO
- Last project structure verification: TODO
- Last test level creation: TODO
- Last PIE/manual verification: TODO

## Risks

- TODO
