...
8. 기획서, 명세, 시뮬레이션 시나리오, 검증 기준 같은 프로젝트 문서가 있으면 루트 `ProjectDocs/`에 두고 `Harness/config/docs.json`의 참조 경로를 맞춘다.
...
- 기존 `Harness/cycles/`
- 기존 프로젝트 문서 폴더 예: `ProjectDocs/`, `Docs/`, `DesignDocs/`
...
- 구버전 `Harness/doc/`에 기획서나 명세가 있으면 삭제하지 말고 루트 `ProjectDocs/` 같은 프로젝트 문서 폴더로 옮긴 뒤 `Harness/config/docs.json`에 경로를 등록한다.
...
- `docs.json`은 프로젝트 문서의 위치와 참조 정책만 담고, 실제 기획서나 명세는 `Harness/` 밖에 둔다.
...
- 구버전 `Harness/doc/` 안의 문서를 `ProjectDocs/` 같은 루트 문서 폴더로 옮겨야 하는지
...
- `Harness/config/docs.json`과 `ProjectDocs/README.md` 참조 정책 확인
...
- 구버전 `Harness/doc/`를 옮겼다면 새 프로젝트 문서 위치와 `docs.json` 갱신 여부

## 8. 템플릿 사용 시 주의

- 이 템플릿은 프로젝트 설명서가 아니라 작업 운영 템플릿이다.
- 루트 `README.md`는 기존 내용을 보존하면서 현재 프로젝트 기준으로 갱신한다.
- 기획서, 구현기준서, 시뮬레이션 시나리오, 검증 기준은 `Harness/` 안에 넣지 않고 루트 `ProjectDocs/` 같은 프로젝트 문서 폴더에 둔다.
